...
describe("parseInput — invalid input", () => {
	it("throws when input is empty", () => {
		expect(() => parseInput([])).toThrow("Input must contain at least a header line.");
	});
	...
	it("throws when header line has non-numeric base cost", () => {
		expect(() => parseInput(["abc 1", "PKG1 5 5"])).toThrow('Invalid header line: "abc 1"');
	});

	it("throws when header line has non-numeric package count", () => {
		expect(() => parseInput(["100 abc", "PKG1 5 5"])).toThrow('Invalid header line: "100 abc"');
	});
	...
	it("throws when package line has non-numeric weight", () => {
		expect(() => parseInput(["100 1", "PKG1 abc 5"])).toThrow('Invalid package line: "PKG1 abc 5"');
	});

	it("throws when package line has non-numeric distance", () => {
		expect(() => parseInput(["100 1", "PKG1 5 abc"])).toThrow('Invalid package line: "PKG1 5 abc"');
	});
});
