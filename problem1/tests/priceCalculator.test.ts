...
describe("calculateDeliveryCost — sample input", () => {
  it("PKG1: weight fails OFR001 criteria → no discount", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "PKG1", discount: 0, totalCost: 175 });
  });
  ...
  it("PKG2: distance fails OFR002 criteria → no discount", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "PKG2", discount: 0, totalCost: 275 });
  });
  ...
  it("PKG3: both criteria met for OFR003 → 5% discount", () => {
    ...
    // Total = 700 - 35 = 665
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "PKG3", discount: 35, totalCost: 665 });
  });
});
...
describe("calculateDeliveryCost — OFR001", () => {
  it("applies 10% when both distance <200 and weight 70–200 are met", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "A", discount: 160, totalCost: 1440 });
  });
  ...
  it("does not apply when distance equals 200 (boundary: strict <200 required)", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "B", discount: 0, totalCost: cost });
  });
  ...
  it("does not apply when weight is below min (69kg)", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "C", discount: 0, totalCost: cost });
  });
});
...
describe("calculateDeliveryCost — OFR002", () => {
  it("applies 7% when distance 50–150 and weight 100–250", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "D", discount: 147, totalCost: 1953 });
  });
  ...
  it("does not apply when weight exceeds max (251kg)", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "E", discount: 0, totalCost: cost });
  });
});
...
describe("calculateDeliveryCost — OFR003", () => {
  it("applies 5% at exact boundary values (distance=50, weight=10)", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "F", discount: 23, totalCost: 427 });
  });
  ...
  it("does not apply when weight is below min (9kg)", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "G", discount: 0, totalCost: cost });
  });
});
...
describe("calculateDeliveryCost — edge cases", () => {
  it("unknown offer code → no discount", () => {
    ...
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "H", discount: 0, totalCost: cost });
  });

  it("empty offer code → no discount", () => {
    const pkg: Package = { id: "I", weight: 100, distance: 100, offerCode: "" };
    const cost = 100 + 1000 + 500; // 1600
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "I", discount: 0, totalCost: cost });
  });

  it("zero weight and distance → only base cost", () => {
    const pkg: Package = { id: "J", weight: 0, distance: 0, offerCode: "" };
    expect(calculateDeliveryCost(BASE, pkg, OFFER_CODES, mockConfig)).toEqual({ id: "J", discount: 0, totalCost: 100 });
  });
